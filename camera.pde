...
pt fromMouse() { 
  ...
  return new pt((float)mousePosArr[0],(float)mousePosArr[1],(float)mousePosArr[2]);
}
...
class cam { 
 ...
 //   println(); println("sD="+sD); this.write(); 
  ...
  void zoom() {
    float r=width/Current.Rbox/2; float x=-(mouseX-px)/r;   float y=-(mouseY-py)/r; 
 //   E.setToPoint(sE); E.addScaledVec(x,I); 
    D=sD+y; 
    };
   
  void track (cam K) {float s=0.0005*Current.nt; if (s<0.005) {s=0.005;}; if (s>0.25) {s=0.25;}; E.moveTowards(s,K.E); F.moveTowards(s,K.F); this.recomputeD(); U.addScaled(s,K.U); U.makeUnit(); this.pose(); this.anchor();}
